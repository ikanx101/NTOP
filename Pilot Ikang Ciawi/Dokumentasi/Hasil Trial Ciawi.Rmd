...
printing = function(temp){
  ...
  temp = 
    temp %>% 
    select(sales_order,nama_toko,armada_ke,armada,order_kubikasi,order_tonase,provinsi,kota_kab,kecamatan) %>% 
    arrange(armada) %>% 
    rename(jenis_armada = armada)
    
  armada_ke = temp$armada_ke %>% unique() %>% sort()
  ...
  merge(temp,df_refer) %>% 
    ...
    arrange(jenis_armada,armada_ke) %>% 
...
## Keterbatasan Titik Pengiriman

Jika kita perhatikan data detail jenis armada yang diberikan, terlihat bahwa __semua jenis armada__ memiliki keterbatasan:

> __Hanya bisa mengantarkan order ke lima titik pengantaran saja__.

Sedangkan _sales order_ (baik dalam kubikasi dan tonase) tidak pernah ada yang bernilai besar. Akibatnya keunggulan armada-armada besar justru tidak dapat diutilisasi dengan baik. Oleh karena itu __tidak ada alasan kita perlu menggunakan armada yang lebih besar dari CARRY dan CDE__. Pada model sekarang ini, saya batasi pengiriman menjadi Carry dan CDE saja. 

Namun demikian, saat tidak ada pembatasan armada pengiriman, saya menemukan penggunaan armada besar tetap memberikan nilai _cost_ yang masih "kecil". Salah satu penyebabnya adalah besaran unit `cost per km` yang digunakan relatif kecil dan tidak terlalu berbeda antara satu dengan yang lain.
